...
public class GetAllPetsWithPaginationHandler(
    IVolunteersReadDbContext volunteersReadDbContext) : IQueryHandler<PagedList<PetDto>, GetAllPetsWithPaginationQuery>
{
    public async Task<Result<PagedList<PetDto>, ErrorList>> Execute(GetAllPetsWithPaginationQuery query,
        CancellationToken token = default)
    {
        var petsQuery = ApplyFilters(volunteersReadDbContext.Pets, query); 
